...
7. Extract the Pipeline's outputs from the workspace and convert them
   into "narrow" format, with output labels dictated by the pipe's
...
class PipelineEngine(with_metaclass(ABCMeta)):
    ...
    def run_pipeline(self, pipeline, start_date, end_date, hooks=None):
        ...
        Parameters
        ----------
        pipeline : zipline.Pipeline.pipe
            The Pipeline to run.
        ...
        hooks : list[implements(PipelineHooks)], optional
            Hooks for instrumenting pipe execution.
...
class SimplePipelineEngine(PipelineEngine):
    ...
    def _compute_root_mask(self, domain, start_date, end_date, extra_rows):
        ...
        if start_date not in sessions:
            raise ValueError(
                "pipe start date ({}) is not a trading session for "
                "domain {}.".format(start_date, domain)
            )

        elif end_date not in sessions:
            raise ValueError(
                "pipe end date {} is not a trading session for "
        ...
        if start_idx < extra_rows:
            raise NoFurtherDataError.from_lookback_window(
                initial_message="Insufficient data to compute pipe:",
    ...
    def compute_chunk(self,
                      ...
                      hooks):
        """
        Compute the pipe terms in the graph for the requested start and end
        dates.
    ...
    def resolve_domain(self, pipeline):
        ...
        if domain is GENERIC:
            raise ValueError(
                "Unable to determine domain for pipe.\n"
                "Pass domain=<desired domain> to your pipe to set a "
                "domain."
            )
        return domain
