...
class Filter(RestrictedDTypeMixin, ComputableTerm):
    """
    pipe expression computing a boolean output.

    Filters are most commonly useful for describing sets of assets to include
    or exclude for some particular purpose. Many pipe API functions accept
    ...
    **either** of its inputs produced True.
    ...
    Filters may be set as the ``screen`` attribute of a pipe, indicating
    asset/date pairs for which the filter produces False should be excluded
    from the pipe's output.  This is useful both for reducing noise in the
    output of a pipe and for reducing memory consumption of pipe
    results.
    """
...
class AllPresent(CustomFilter, SingleInputMixin, StandardOutputs):
    """pipe filter indicating input term has data for a given window.
    """
    def _validate(self):
...
class MaximumFilter(Filter, StandardOutputs):
    """pipe filter that selects the top asset, possibly grouped and masked.
    """
    window_length = 0
