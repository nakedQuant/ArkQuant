"""pipe hooks for tracking and displaying progress.
"""
import cgi
    ...
    def running_pipeline(self, pipeline, start_date, end_date):
        ...
        try:
            ...
            if self._model is None:
                # This will only happen if an error happens in the pipe
...
class ProgressModel(object):
    """
    Model object for tracking progress of a pipe execution.

    Parameters
    ----------
    nterms : int
        Number of terms in the execution plan of the pipe being run.
...
class IPythonWidgetProgressPublisher(object):
    ...
    def publish(self, model):
        if model.state == 'init':
            self._heading.value = '<b>Analyzing pipe...</b>'
            ...
            self._heading.value = (
                "<b>Running pipe</b>: Chunk Start={}, Chunk End={}"
            ...
            display(
                IPython_HTML("<b>pipe Execution Time:</b> {}".format(
    ...
    def _format_execution_time(total_seconds):
        """Helper method for displaying total execution time of a pipe.
