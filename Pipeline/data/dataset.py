...
class BoundColumn(LoadableTerm):
    ...
    class. pipe API users should never construct instances of this
    directly.
    """
    mask = AssetExists()
    window_safe = True
    ...
    def graph_repr(self):
        """Short repr to use when rendering pipe graphs."""
...
class DataSet(with_metaclass(DataSetMeta, object)):
    ...
    To create a new pipe dataset, define a subclass of :class:`DataSet` and
    ...
    in a pipe running on **any** domain.
...
class DataSetFamily(with_metaclass(DataSetFamilyMeta)):
    """
    Base class for pipe dataset families.
