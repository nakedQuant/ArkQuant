"""
Blaze integration with the pipe API.

For an overview of the blaze project, see blaze.pydata.org

The blaze loader for the pipe API is designed to allow us to load
...
The blaze pipe API loader expects that data is formatted in a tabular way.
...
One of the key features of the pipe API is the handling of adjustments and
...
Conversion from Blaze to the pipe API
-----------------------------------------

Now that our data is structured in the way that the blaze loader expects, we
are ready to convert our blaze expressions into pipe API objects.
...
each field of the record. Some datashape types cannot be coerced into pipe
...
Using our pipe DataSets and Columns
---------------------------------------

Once we have mapped our blaze expressions into pipe API objects, we may
use them just like any other datasets or columns. For more information on how
to run a Pipeline or using the pipe API, see:
...
class NonPipelineField(InvalidField):
    error_format = (
        "field '{field}' was a non pipe API compatible type: '{type_}'"
    )
...
def from_blaze(expr,
    ...
    """Create a pipe API object from a blaze expression.
    ...
    # Create or retrieve the pipe API dataset.
