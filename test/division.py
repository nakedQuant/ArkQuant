...
class BaseDivision(object):
    ...
    @staticmethod
    def _init_division_data(asset, dts):
        tick_size = asset.tick_size
        open_change, pre_close = portal.get_open_pct(asset, dts)
        per_capital = pre_close * (1 + asset.restricted) * tick_size
        per_size = np.ceil(20000 / per_capital) * tick_size
        return open_change, pre_close, per_capital, per_size
...
class CapitalDivision(BaseDivision):
    ...
    def __init__(self,
                 distribution,
                 control_models):
        super(CapitalDivision, self).__init__(dist=distribution,
                                              controls=control_models)

    def yield_size_on_capital(self, asset, capital, portfolio, dts):
        """
            针对于买入操作
        ...
        """
        open_pct, pre_close, base_capital, per_size = super()._init_division_data(asset, dts)

        # size = capital / (pre_close * (1 + asset.restricted)) if asset.increment else

        # size per_size 单位手
        size = asset.tick_size * np.floor(capital / base_capital)
        control_size = self.trade_controls.validate(asset, size, portfolio, dts)
        """根据目标q --- 生成size序列拆分订单数量"""
        size_array = np.tile([per_size], int(control_size/per_size))
        size_array[np.random.randint(0, len(size_array), control_size % per_size)] += 1
        size_array = size_array * asset.tick_size * -1
        # simulate price
        dist = self.dist.simulate_dist(int(control_size / per_size), open_pct)
        clip_pct = np.clip(dist, (1 - asset.restricted), (1 + asset.restricted))
        sim_prices = clip_pct * pre_close
        return zip(sim_prices, size_array)

    def yield_ticker_on_capital(self, asset, capital, portfolio, dts):
        """
            针对于买入操作
            a. 计算满足最低capital(基于手续费逻辑），同时计算size
            b. 存在竞价机制 --- 基于size设立时点order
        ...
        size_array = size_array * asset.tick_size * -1
        # simulate ticker
        sim_tickers = self.dist.simulate_ticker(int(control_size / per_size))
...
class PositionDivision(BaseDivision):
    ...
    def __init__(self,
                 distribution,
                 control_models):
        super(PositionDivision, self).__init__(dist=distribution,
                                               controls=control_models)

    def yield_size_on_position(self, position, portfolio, dts):
        """
            针对于买入操作
            a. 计算满足最低capital(基于手续费逻辑），同时计算size
            b. 存在竞价机制 --- 基于size设立时点order
        ...
        open_pct, pre_close, base_capital, per_size = super()._init_division_data(asset, dts)
        # size per_size 单位手
        size = np.floor(control_amount / per_size)
        """根据目标q --- 生成size序列拆分订单数量"""
    ...
    def yield_ticker_on_position(self, position, portfolio, dts):
        """
            针对于买入操作
            a. 计算满足最低capital(基于手续费逻辑），同时计算size
            b. 存在竞价机制 --- 基于size设立时点order
