...
class TestApi(TraderApi):
    ...
    # data.qty_left字段在订单为未成交、部成、全成、废单状态时，表示此订单还没有成交的数量，在部撤、全撤状态时，表示此订单被撤的数量。
    # data.order_cancel_xtp_id为其所对应的撤单ID，不为0时表示此单被撤成功
    ...
    # 在订单未成交、全部成交、全部撤单、部分撤单、已拒绝这些状态时会有响应，对于部分成交的情况，请由订单的成交回报来自行确认。
    # 所有登录了此用户的客户端都将收到此用户的订单响应
    ...
    # 成交通知
    # @param data 成交回报的具体信息，用户可以通过data.order_xtp_id来管理订单，通过GetClientIDByXTPID() == client_id来过滤自己的订单。
    # 对于上交所，exec_id可以唯一标识一笔成交。当发现2笔成交回报拥有相同的exec_id，则可以认为此笔交易自成交了。
    # 对于深交所，exec_id是唯一的，暂时无此判断机制。report_index+market字段可以组成唯一标识表示成交回报。
    # @param session 资金账户对应的session，登录时得到
    # @remark 订单有成交发生的时候，会被调用，需要快速返回，否则会堵塞后续消息，当堵塞严重时，会触发断线。
    # 所有登录了此用户的客户端都将收到此用户的成交回报。相关订单为部成状态，需要用户通过成交回报的成交数量来确定，OnOrderEvent()不会推送部成状态。
    ...
    def onOptionCombinedTradeEvent(self, data):
        ...
        print("data['order_xtp_id']:", data['order_xtp_id'])#X TP系统订单ID，无需用户填写，在XTP系统中唯一
        print("data['order_client_id']:", data['order_client_id'])# 报单引用，用户自定义
        print("data['market']:", data['market'])# 交易市场
        print("data['local_order_id']:", data['local_order_id'])# 订单号，引入XTPID后，该字段实际和order_xtp_id重复。接口中暂时保留
        print("data['side']:", data['side'])# 组合方向
...
if __name__ == '__main__':
    ...
    # @remark 该方法要在Login方法前调用。若不调用则不会收到公共流的数据。注意在用户断线后，如果不登出就login()，公共流订阅方式不会起作用。
    # 用户只会收到断线后的所有消息。如果先logout()再login()，那么公共流订阅方式会起作用，用户收到的数据会根据用户的选择方式而定。
